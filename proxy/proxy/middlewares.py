...
def rotate_ip():
    ...
    """
    # Method 2 (stem package)
    tor_ip_changer = TorIpChanger(tor_password='mytor', tor_port=9051, local_http_proxy='127.0.0.1:8118')
...
class ProxyMiddleware(HttpProxyMiddleware):
    def process_response(self, request, response, spider):
        ...
        if not response.url.endswith('robots.txt') and response.status != 200:
            rotate_ip()
            return request
        return response
